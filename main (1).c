...
static const uint8_t tx_nonce[12] = {
    ...
    0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00,
}; // MCXC444 -> ESP32   (this board's tx_nonce, ESP32's rx_nonce)
static const uint8_t rx_nonce[12] = {
    0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x01,
}; // ESP32  -> MCXC444  (this board's rx_nonce, ESP32's tx_nonce)
...
void sendMessage(const char *msg) {
    if (xSemaphoreTake(txctrMutex, portMAX_DELAY) == pdTRUE) {
        uint8_t frame[64] = {0};
		...
		if(xQueueReceive(alarm_queue, (AlarmEvent_t *) &event, portMAX_DELAY) == pdTRUE){
			...
			case(STATE_SET_TIMER):
				if(event == EVT_JOY_RIGHT){
                    // edit minutes (ESP32 handles value via JOY:RIGHT)
				} else if (event == EVT_JOY_LEFT) {
                    // edit hours (ESP32 handles value via JOY:LEFT)
					...
					state = STATE_IDLE;
                    // cancel timer and go back idle
				} else if (event == EVT_JOY_LONG_PRESS) {
                    state = STATE_IDLE;
                    // confirm timer and start countdown
                }
                break;
...
int main(void)
{
    ...
    txctrMutex = xSemaphoreCreateMutex();
