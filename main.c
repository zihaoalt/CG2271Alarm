...
volatile AlarmState_t currentAlarmState = STATE_IDLE;
...
void vTaskButtonHandler(void *pv)
{
    btn_evt_t evt;
...
void vTaskJoystick(void *pv)
{
    for (;;) {
        ...
        if (x < (JOY_CENTRE - JOY_DEAD)) {
            PRINTF("LEFT\r\n");
            AlarmEvent_t evt = EVT_JOY_LEFT;
            xQueueSend(alarm_queue, &evt, portMAX_DELAY);
            sendMessage("JOY:LEFT\n");
        } else if (x > (JOY_CENTRE + JOY_DEAD)) {
            PRINTF("RIGHT\r\n");
            AlarmEvent_t evt = EVT_JOY_RIGHT;
            xQueueSend(alarm_queue, &evt, portMAX_DELAY);
            sendMessage("JOY:RIGHT\n");
        } else if (y < (JOY_CENTRE - JOY_DEAD)) {
            PRINTF("DOWN\r\n");
            AlarmEvent_t evt = EVT_JOY_DOWN;
            xQueueSend(alarm_queue, &evt, portMAX_DELAY);
            sendMessage("JOY:DOWN\n");
        } else if (y > (JOY_CENTRE + JOY_DEAD)) {
            PRINTF("UP\r\n");
            AlarmEvent_t evt = EVT_JOY_UP;
            xQueueSend(alarm_queue, &evt, portMAX_DELAY);
            sendMessage("JOY:UP\n");
        }
		...
		if(xQueueReceive(alarm_queue, (AlarmEvent_t *) &event, portMAX_DELAY) == pdTRUE){
			...
			case(STATE_ALARM_FIRING):
				if (event == EVT_SW2) {
                    ...
                    if (prev_state == STATE_ALARM_FIRING) {
                        state = STATE_IDLE;
                    }
				}
                break;
	...
	while (1) {
		...
		if(xQueueReceive(send_queue, (TMessage *) &msg, portMAX_DELAY) == pdTRUE) {
			...
			if (xSemaphoreTake(sendSema, portMAX_DELAY) == pdTRUE) {
				strncpy(send_buffer, msg.message, MAX_MSG_LEN - 1);
...
int main(void)
{
    ...
    xTaskCreate(initRTCTask, "initRTCTask", configMINIMAL_STACK_SIZE + 100, NULL, 5, NULL);
    xTaskCreate(alarmFSMTask, "alarmFSMTask", configMINIMAL_STACK_SIZE + 100, NULL, 4, NULL);
    xTaskCreate(sendTask, "sendTask", configMINIMAL_STACK_SIZE + 100, NULL, 3, NULL);
    xTaskCreate(recvTask, "recvTask", configMINIMAL_STACK_SIZE + 100, NULL, 3, NULL);
    xTaskCreate(vTaskButton, "btn", 256, NULL, 3, NULL);
    xTaskCreate(sw2Task, "sw2Task", configMINIMAL_STACK_SIZE + 100, NULL, 2, NULL);
    xTaskCreate(vTaskButtonHandler, "btn_hdl", 256, NULL, 2, NULL);
    xTaskCreate(vTaskJoystick, "joy", 256, NULL, 2, NULL);
    xTaskCreate(ledIndicatorTask, "ledIndicatorTask", configMINIMAL_STACK_SIZE + 100, NULL, 1, NULL);
